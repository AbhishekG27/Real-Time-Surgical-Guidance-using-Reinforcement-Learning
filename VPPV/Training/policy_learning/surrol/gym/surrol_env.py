...
class SurRoLEnv(gym.Env):
    ...
    def __init__(self, render_mode: str = None, cid: int = -1):
        ...
        if render_mode == 'human':
            ...
            # Try to load EGL renderer for headless rendering, but fail gracefully if it doesn't work
            try:
            if socket.gethostname().startswith('pc') or True:
                ...
                plugin = p.loadPlugin(egl.get_filename(), "_eglRendererPlugin")
                        if plugin < 0:
                            print("Warning: Failed to load EGL renderer plugin, continuing without it")
            except Exception as e:
                print(f"Warning: Could not load EGL renderer ({e}), continuing without hardware rendering")
