...
# Ensure SurRoL environments are registered with gym (NeedleGrasp-Traj-v0, etc.)
...
class RLTrainer(BaseTrainer):
    ...
    def _setup_env(self):
        # Ensure SurRoL environments are registered with Gym.
        # The SurRoL package installed from VPPV/Training/data_generation
        # exposes its gym registration via `surrol.gym`.
        import surrol.gym  # noqa: F401

        # Create environment in headless mode (no rendering) for training to avoid segfaults
        self.train_env = gym.make(self.cfg.task, render_mode=None)
    ...
    def _setup_agent(self):
        ...
        # Move agent to the configured device (CPU or CUDA)
    ...
    def _setup_misc(self):
        ...
        if self.is_chef:
            ...
            # Don't clear model directory when resuming training
    ...
    def train(self):
        ...
        n_eval_episodes = int(n_train_episodes / self.cfg.n_eval) if self.cfg.n_eval > 0 else 0
        n_save_episodes = int(n_train_episodes / self.cfg.n_save) if self.cfg.n_save > 0 else 0
        n_log_episodes = int(n_train_episodes / self.cfg.n_log) if self.cfg.n_log > 0 else 0
        ...
        if n_eval_episodes > 0 and n_save_episodes > 0 and n_save_episodes % n_eval_episodes != 0:
            n_save_episodes = int(n_save_episodes / n_eval_episodes) * n_eval_episodes
        
        # Ensure minimum checkpoint saving frequency to avoid losing progress
        # Save at least every 10 episodes or every 5000 steps, whichever comes first
        MIN_SAVE_EPISODES = 10
        MIN_SAVE_STEPS = 5000
        if n_save_episodes == 0 or n_save_episodes > MIN_SAVE_EPISODES:
            n_save_episodes = MIN_SAVE_EPISODES
        # Also ensure we save more frequently to avoid losing too much progress
        # Save every 5 episodes as a safety measure
        SAFETY_SAVE_EPISODES = 5
        if n_save_episodes > SAFETY_SAVE_EPISODES:
            n_save_episodes = SAFETY_SAVE_EPISODES
        ...
        save_every_steps = Every(MIN_SAVE_STEPS)  # Also save based on steps
        ...
        last_saved_step = 0  # Track last checkpoint save step
        last_saved_episode = 0  # Track last checkpoint save episode
        if self.is_chef:
        ...
        while train_until_episode(self.global_episode):
            ...
            if self.global_step > self.cfg.n_seed_steps and eval_every_episodes(self.global_episode):
                ...
                if score>self.best_score:
                    ...
                    last_saved_episode = self.global_episode

            # Save checkpoint if either episode-based or step-based condition is met
            should_save = False
            if not self.cfg.dont_save and self.is_chef:
                # Save based on episodes
                if save_every_episodes(self.global_episode) and self.global_episode != last_saved_episode:
                    should_save = True
                # Also save based on steps (to ensure frequent saves even if episodes are slow)
                elif save_every_steps(self.global_step) and self.global_step - last_saved_step >= MIN_SAVE_STEPS:
                    should_save = True
                # Safety save: if we haven't saved in a while (more than 20 episodes), force a save
                elif self.global_episode - last_saved_episode >= 20:
                    should_save = True
                    self.termlog.info(f'Safety save triggered: {self.global_episode - last_saved_episode} episodes since last save')
            ...
            if should_save:
                ...
                self.termlog.info(f'Save checkpoint to {os.path.join(self.model_dir, filename)} (episode={self.global_episode}, step={self.global_step})')
                last_saved_step = self.global_step
                last_saved_episode = self.global_episode
    ...
    def resume_training(self):
        # Use model_dir as default if ckpt_dir is not specified
        ckpt_dir = self.cfg.ckpt_dir if self.cfg.ckpt_dir else str(self.model_dir)
        
        # If ckpt_episode is not specified, default to 'latest'
        ckpt_episode = self.cfg.ckpt_episode if hasattr(self.cfg, 'ckpt_episode') and self.cfg.ckpt_episode else 'latest'
        
        try:
            checkpt = CheckpointHandler.load_checkpoint(
                    ckpt_dir, self.agent, self.device, ckpt_episode
                )
            
            # Restore training state from checkpoint
            if 'global_step' in checkpt:
                self._global_step = checkpt['global_step']
                self.termlog.info(f'Resuming from global_step: {self._global_step}')
            else:
                self.termlog.warning('Checkpoint does not contain global_step, starting from step 0')
                
            if 'episode' in checkpt:
                self._global_episode = checkpt['episode']
                self.termlog.info(f'Resuming from episode: {self._global_episode}')
            else:
                self.termlog.warning('Checkpoint does not contain episode, starting from episode 0')
                
            # Also restore buffer state if available
            if 'buffer_state' in checkpt:
                self.termlog.info('Restoring replay buffer state from checkpoint')
                # Buffer restoration would go here if needed
                
        except (ValueError, FileNotFoundError) as e:
            self.termlog.error(f"Failed to resume training: {e}")
            self.termlog.error(f"Checkpoint directory: {ckpt_dir}")
            self.termlog.error("No checkpoint found. To start training from scratch, remove 'resume_training=True' from the command.")
            raise
