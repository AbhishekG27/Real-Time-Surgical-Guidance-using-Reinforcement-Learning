...
class CheckpointHandler:
    ...
    def get_resume_ckpt_file(resume, path):
        if resume == 'latest':
            ...
            if len(episodes) == 0:
                raise ValueError(f"No checkpoints found at {path}. Cannot resume with 'latest' checkpoint.")
            ...
            # Assume resume is a numeric episode number
            try:
                max_episode = int(resume)
                resume_file = CheckpointHandler.get_ckpt_name(max_episode)
            except (ValueError, TypeError):
                # Fallback to treating as string episode name
                max_episode = 0
                resume_file = CheckpointHandler.get_ckpt_name(str(resume))
        
        checkpt_path = os.path.join(path, resume_file)
        if not os.path.exists(checkpt_path):
            raise FileNotFoundError(f"Checkpoint file not found: {checkpt_path}. Available checkpoints: {CheckpointHandler.get_episode(path)}")
        
        return checkpt_path, max_episode
    ...
    def load_checkpoint(checkpt_dir, agent, device, episode='best'):
        """Loads weigths from checkpoint. Returns checkpoint dict for restoring training state."""
