...
class DualDDPG(BaseAgent):
    '''
    Dual Deep Deterministic Policy Gradient
    Uses two Q-networks and averages them for both action selection and target computation.
    This is a simpler variant that uses ensemble averaging instead of minimum.
    '''
    def __init__(
        self,
        env_params,
    ...
    def update_critic(self, obs, action, reward, next_obs):
        with torch.no_grad():
            ...
            # Use average of two Q-networks for target (Dual Q-learning)
            target_Q1, target_Q2 = self.critic_target(next_obs, action_out)
            target_V = (target_Q1 + target_Q2) / 2.0  # Average instead of minimum
        ...
        metrics = AttrDict(
            ...
            critic_q_avg=(Q1 + Q2).mean().item() / 2.0,
    ...
    def update_actor(self, obs):
        ...
        # Use average Q-value for actor update
        Q1, Q2 = self.critic(obs, action_out)
        Q_out = (Q1 + Q2) / 2.0  # Average instead of minimum
