...
class DoubleDDPG(BaseAgent):
    '''
    Double Deep Deterministic Policy Gradient
    Uses two Q-networks and takes the minimum for target computation to reduce overestimation bias.
    Similar to TD3 but without delayed policy updates and target policy smoothing.
    Refer to: https://arxiv.org/abs/1509.02971 (DDPG) and https://arxiv.org/abs/1802.09477 (TD3)
    '''
    def __init__(
        self,
        env_params,
        ...
        # Use DoubleCritic (two Q-networks)
    ...
    def update_critic(self, obs, action, reward, next_obs):
        with torch.no_grad():
            ...
            # Use minimum of two Q-networks for target (Double Q-learning)
            target_V = self.critic_target.q(next_obs, action_out)  # This uses min(q1, q2)
        ...
        metrics = AttrDict(
            ...
            critic_q_min=torch.min(Q1, Q2).mean().item(),
    ...
    def update_actor(self, obs):
        ...
        # Use minimum Q-value for actor update
        Q_out = self.critic.q(obs, action_out)  # Uses min(q1, q2)
